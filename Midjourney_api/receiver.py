    ...
    def downloading_results(self):
        ...
        for i in self.df.index:
            if self.df.loc[i].is_downloaded == 0:
                ...
                # Extract prompt name from the message
                prompt_name = self.df.loc[i].prompt.replace(' ', '_')
                
                # Create a new filename based on the prompt name
                filename = f"{prompt_name}.jpg"  # You can change the extension if needed
                
                # Save the image with the new filename
                with open(os.path.join(self.local_path, filename), "wb") as req:
                    req.write(response.content)
                    
                # Update the DataFrame with the new filename
                self.df.loc[i, 'filename'] = filename
                
                # Mark the prompt as downloaded
                self.df.loc[i, 'is_downloaded'] = 1
